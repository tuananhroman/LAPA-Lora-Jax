...
class VideoLLaMAConfig(LLaMAConfig):
    ...
    def __init__(self, vision_vocab_size=8448, tie_vision_embeddings=False, delta_vocab_size=32, action_vocab_size=245, sample_mode='all', lora_rank=0, lora_alpha=16.0, use_rslora=False, **kwargs):
        super().__init__(lora_rank=lora_rank, lora_alpha=lora_alpha, use_rslora=use_rslora, **kwargs)
...
class FlaxDeltaActionLaMAForCausalLMModule(nn.Module):
    ...
    def setup(self):
        self.transformer = FlaxVideoLLaMAModule(self.config, dtype=self.dtype, param_dtype=self.param_dtype, precision=self.precision)
