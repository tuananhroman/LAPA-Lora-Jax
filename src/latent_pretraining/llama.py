        ...
        if scan_layers:
            if scan_axis == 0:
                return (
                    ...
                    ("(attention|feed_forward)/(wq|wk|wv|wo|w1|w2|w3)/lora_A", PS(None, None, None)),
                    ("(attention|feed_forward)/(wq|wk|wv|wo|w1|w2|w3)/lora_B", PS(None, None, None)),
                    ('.*', PS(None)),
                )
            elif scan_axis == 1:
                return (
            ...
            return (
                ...
                ("transformer/ln_f/kernel", PS(None)),
                ("lm_head/kernel", PS(("fsdp", "sp"), "tp")),
                ("(attention|feed_forward)/(wq|wk|wv|wo|w1|w2|w3)/lora_A", PS(None, None)),
                ("(attention|feed_forward)/(wq|wk|wv|wo|w1|w2|w3)/lora_B", PS(None, None)),
                ('.*', PS(None)),
            )
...
class LoRADense(nn.Module):
    """Drop-in replacement for nn.Dense (use_bias=False) with optional LoRA adapter.

    Parameters:
      - `kernel`: (in_features, features) — base weights (frozen for LoRA fine-tune; same name as nn.Dense for ckpt compatibility).
      - `lora_A`: (in_features, rank) — trainable, normal init.
      - `lora_B`: (rank, features) — trainable, zero init (so LoRA delta = 0 at start).
    """
    features: int
    dtype: jnp.dtype = jnp.float32
    param_dtype: jnp.dtype = jnp.float32
    ...
    def __call__(self, x):
        ...
        # kernel parameter so XLA does not allocate huge full-shape gradient buffers for
        # it during backprop (gradients still flow through `x` to reach earlier LoRA params).
        kernel_used = jax.lax.stop_gradient(kernel) if self.lora_rank > 0 else kernel
        y = jnp.matmul(x, kernel_used.astype(self.dtype), precision=self.precision)
        if self.lora_rank > 0:
            lora_a = self.param(
                'lora_A', jax.nn.initializers.normal(stddev=1.0 / max(self.lora_rank, 1)),
                (in_features, self.lora_rank), self.param_dtype,
            )
            lora_b = self.param(
